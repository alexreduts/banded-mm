...
def _xGBMM_inner(
    ...
    E123 = [cp.empty((E.shape[0], E.shape[1])) for _ in range(2)]
    ...
    for i in range(1, num_blocks):
        ...
        with streams[i%2] as stream:
            ...
            if A1_cur.stop > A1_cur.start:
                E123[i%2][A1_cur, :] = E123[i%2][A1_cur, :] + A11[i%2][:A1_cur.stop-A1_cur.start,:D1_cur.stop-D1_cur.start] @ D1[0][:D1_cur.stop-D1_cur.start]

            if A2_cur.stop > A2_cur.start:
                E123[i%2][A2_cur, :] = E123[i%2][A2_cur, :] + A21[i%2][:A2_cur.stop-A2_cur.start,:D1_cur.stop-D1_cur.start] @ D1[0][:D1_cur.stop-D1_cur.start]

            if A3_cur.stop > A3_cur.start:
                E123[i%2][A3_cur, :] = E123[i%2][A3_cur, :] + A31[i%2][:A3_cur.stop-A3_cur.start,:D1_cur.stop-D1_cur.start] @ D1[0][:D1_cur.stop-D1_cur.start]
