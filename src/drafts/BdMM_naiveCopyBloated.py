...
from archive.naive_mm import naive_banded_mm


# General blocked banded matrix multiplication (Only A is banded)
def _gbmm_BLK_outer(
    C: np.ndarray, A: np.ndarray, B: np.ndarray, ku: int, kl: int
) -> np.ndarray:
    # Partition
    CL_ = slice(0, 0)
...
def _gbmm_BLK_inner(
    E: np.ndarray, A: np.ndarray, D: np.ndarray, ku: int, kl: int
) -> np.ndarray:
    # Partition
    ET_ = slice(0, 0)
...
def gbmm_BLK(C: np.ndarray, A: np.ndarray, B: np.ndarray, ku: int, kl: int):
