...
class TestxGBMM(TestCase):
    ...
    def test_dense(self):
        ...
        C_rect = xGBMM_streamed(A_rect, 4, 9, B_rect, 9, 4, 3, 2)
        self.assertTrue(
            np.allclose(C_rect, REF_rect),
            "Rectangular Dense Case failed"
        )

        C_square = xGBMM_streamed(A_square, 9, 9, B_square, 9, 9, 3, 2)
        self.assertTrue(
            np.allclose(C_square, REF_square),
    ...
    def test_inner_loop_banded_with_overlap(self):
        ...
        C_square = xGBMM_streamed(A_square, 4, 5, B_square, 4, 5, 10, 2)
        self.assertTrue(
            np.allclose(C_square, REF_square),
