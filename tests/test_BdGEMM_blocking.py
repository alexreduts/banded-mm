...
class TestBdGEMM(TestCase):
    ...
    def test_diagonal(self):
        ...
        C_rect = BdGEMM_blocking(A_rect, 0, 0, B_rect, 0, 0, 3, 2)
        self.assertTrue(
            np.allclose(C_rect, REF_rect),
            "Rectangular Diagonal Case failed"
        )

        C_square = BdGEMM_blocking(A_square, 0, 0, B_square, 0, 0, 3, 2)
        self.assertTrue(
    ...
    def test_dense(self):
        ...
        C_rect = BdGEMM_blocking(A_rect, 4, 9, B_rect, 9, 4, 3, 2)
        self.assertTrue(
            np.allclose(C_rect, REF_rect),
            "Rectangular Dense Case failed"
        )

        C_square = BdGEMM_blocking(A_square, 9, 9, B_square, 9, 9, 3, 2)
        self.assertTrue(
            np.allclose(C_square, REF_square),
    ...
    def test_banded(self):
        ...
        C_rect = BdGEMM_blocking(A_rect, 2, 3, B_rect, 2, 3, 3, 2)
        self.assertTrue(
            np.allclose(C_rect, REF_rect),
            "Rectangular Diagonal Case failed"
        )

        C_square = BdGEMM_blocking(A_square, 2, 3, B_square, 2, 3, 3, 2)
        self.assertTrue(
            np.allclose(C_square, REF_square),
            "Squared Diagonal Case failed"
        )
    ...
    def test_inner_loop_banded_no_overlap(self):
        ...
        C_rect = BdGEMM_blocking(A_rect, 1, 2, B_rect, 1, 2, 5, 2)
        self.assertTrue(
            np.allclose(C_rect, REF_rect),
            "Rectangular Inner Loop Banded NO Overlap failed"
        )

        C_square = BdGEMM_blocking(A_square, 2, 3, B_square, 2, 3, 10, 2)
        self.assertTrue(
            np.allclose(C_square, REF_square),
    ...
    def test_inner_loop_banded_with_overlap(self):
        ...
        C_rect = BdGEMM_blocking(A_rect, 2, 3, B_rect, 2, 3, 5, 2)
        self.assertTrue(
            np.allclose(C_rect, REF_rect),
            "Rectangular Inner Loop Banded With Overlap failed"
        )

        C_square = BdGEMM_blocking(A_square, 4, 5, B_square, 4, 5, 10, 2)
        self.assertTrue(
            np.allclose(C_square, REF_square),
